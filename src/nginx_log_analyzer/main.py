...
# TODO: Вынести конфигурацию логгера в отдельный модуль
import structlog
...
def main() -> None:
    ...
    try:
        ...
        log.error('Ошибка: Файл конфигурации не найден', path=args.config, error=str(e))
        sys.exit(1)
    except ValueError as e:
        log.error('Ошибка: Некорректный формат файла конфигурации', error=str(e))
        sys.exit(1)
    except Exception as e:
        log.error('Неизвестная ошибка при загрузке конфигурации', error=str(e))
        sys.exit(1)

    log.info('Конфигурация загружена', config=config)

    # 2. Ищем последний доступный лог-файл
    log.info('Поиск последнего лог-файла', log_dir=str(config.log_dir))
    log_metadata = find_latest_log(config.log_dir)

    if not log_metadata:
        log.warning('Нет доступных лог-файлов для анализа')
        sys.exit(0)

    log.info('Обнаружен лог-файл', file=str(log_metadata.path), file_type=log_metadata.file_type.value)
    ...
    if report_path.exists():
        log.info('Отчет уже существует, повторная генерация не требуется', report_path=str(report_path))
        sys.exit(0)
    ...
    try:
        ...
        if not parsed_entries:
            log.warning('Файл журнала пуст, отчет не будет создан')
            sys.exit(0)

        log.info(f'Успешно обработано {len(parsed_entries)} записей')
        ...
        log.info('Отчет успешно создан', report_path=str(report_path), report_name=report_filename)

    except LogParsingError as e:
        log.error('Ошибка парсинга логов', error=str(e))
        sys.exit(1)
    except Exception as e:
        log.error('Неизвестная ошибка при разборе логов', error=str(e))
        sys.exit(1)
